...
- [ ] P0-1 `data/prefecture_longitude.json` 作成（正式スキーマ・47件・09準拠）
- [ ] P0-2 `DataLoader.loadPrefectureLongitude()` 追加
- [ ] P0-3 `DateUtils.addDays` 実装（`03` の `isLeapYear` / `daysInMonth` / 正負ループ。`new Date` 不使用）
- [ ] P0-4 `DateUtils.addMinutes` 実装（`floorDiv` / `mod` 仕様どおり）
- [ ] P0-5 `constants.js` に `SHI_MODE` / `DEFAULT_SHI_MODE` / `JST_REFERENCE_LONGITUDE` / `JST_OFFSET_MS` / `MAX_ABS_OFFSET_MINUTES` 追加
- [ ] P0-6 `DateUtils` の JST 固定API
  - `toJstEpochMillis`
  ...
  - 既存 `getDaysDifference` は廃止（呼び出し置換）
  - `isValidDate` → `daysInMonth` ベース

### テスト（必須）

- [ ] P0-T1 `DateUtils.addMinutes` / `addDays` 単体テスト
  - 同日内加減
  - 前日/翌日跨ぎ（負の剰余）
  - 月末（1/31+1日）
  - 年末（12/31 23:50+20分）
  - 閏日（閏年 2/28+1日）
  - 複数日跨ぎ
- [ ] P0-T2 経度マスタ読込テスト（47件、`referenceMeridianEast===135`、東京 code `13` 存在）
- [ ] P0-T3 **DU-01〜05**（epoch / ISO offset保持 / createDate / 暦日差 / **小数日の経過時間**）
- [ ] P0-T4 既存テストが壊れていないこと（スモーク実行）
...
- [ ] P4-1 `sidepanel.html` 配置順どおりに項目追加（子時・都道府県・時差）
- [ ] P4-2 `sidepanel.css`（`.tz-inputs` / `.help-text` 等）
- [ ] P4-3 `FormRenderer`（bind / get / set / reset / `populatePrefectures`）
- [ ] P4-4 `InputParser`（prefectureCode / offset / shiMode。旧 birthplace 削除）
- [ ] P4-5 `InputManager` 検証
  - 時あり・分空 → 0
  - 時差 ±23:59
  - 都道府県コード存在確認
...
- [ ] P4-T1 InputParser / InputManager 単体テスト
  - 分欠落→0
  - 時差境界・**TC-09（±24:00 はフォームエラー）**
  - 子時フォールバック / 不正値エラー
  - 不正都道府県コード→フォームエラー相当
- [ ] P4-T2 **UI-01** 都道府県47件（マスタ生成後の件数確認。自動または手順書）
- [ ] P4-T3 **UI-02, UI-03** デフォルト値（時差0、子時 switch23）
- [ ] P4-T4 **UI-06** 時のみ入力の正規化（単体で担保）
- [ ] P4-T5 時差・子時ヘルプ文がDOMに存在し、子時切替で文面が変わること
...
- [ ] P6-1 `ResultRenderer` に補正サマリ描画
  - 適用時: 「時刻補正: 適用」+ 内訳
  - 未入力時: 1行メッセージ
  - `switch23`×23時台の注記
- [ ] P6-2 サマリを PNG キャプチャ対象 DOM 配下へ配置
- [ ] P6-3 CSS 調整（可読性）
- [ ] P6-4 `clear()` で `#time-correction-summary` を空・非表示にする

### テスト（必須）

- [ ] P6-T1 サマリHTML生成の単体/スナップ相当（適用・未入力・注記あり）
  - `clear()` 後にサマリDOMが空/非表示であること
- [ ] P6-T2 **UI-04, UI-05** 手動またはDOMアサーション
- [ ] P6-T3 **UI-07** PNGにサマリが写る目視確認（チェックリスト記録）
- [ ] P6-T4 **UI-09** 年跨ぎ時の表示年（結合結果）
...
- [ ] P7-1 `chrome-extension/package.json` → `1.5.0`
- [ ] P7-2 `chrome-extension/sidepanel.html` フッタ → `v1.5.0`
- [ ] P7-3 `chrome-extension/docs/USER_GUIDE.md` 更新（時差・都道府県・子時・23時台日跨ぎ・大運時分・時刻なし12:00）
- [ ] P7-4 `chrome-extension/docs/ALGORITHM.md` 更新（補正式・子時・日時役割分担）

### テスト（必須）

- [ ] P7-T1 **UI-08** フッタ / package バージョン一致確認
- [ ] P7-T2 ドキュメントに東京+19分、`resolveDayPillarDate` 適用範囲、大運時分が記載されていること（チェックリスト）
- [ ] P7-T3 自動テスト一式の再実行（ドキュメント変更による破壊がないこと）
...
- [ ] P8-1 全自動テスト実行・失敗ゼロ
- [ ] P8-2 受入チェックリスト UI-01〜09 を最終確認
- [ ] P8-3 既知差分（23時台日柱、大運時分）の説明がガイドにあることを再確認
- [ ] P8-4（任意）実装メモ / 変更点サマリを design 配下へ残す

### テスト（必須）

- [ ] P8-T1 全スイートグリーン（P0〜P6で追加した全テスト含む）
- [ ] P8-T2 受入 UI-01〜09 すべてチェック
- [ ] P8-T3 回帰観点: 補正なし・非23時台の代表ケースが V1.0 と一致
